...
**보완 결정 (2026-07-06, M9 테스트 피드백):** 종료된 세션 재생은 AVQueuePlayer가 아니라 **AVMutableComposition**(청크들을 하나의 가상 타임라인으로 결합 — 재인코딩 없음)으로: 전체 길이 표시·전 구간 시크 제공. 진행 중 세션(폴백 라이브)만 큐 방식 유지. 합본 파일 생성·재업로드는 금지 — CKAsset은 불변이라 실시간 합본은 구조적으로 불가하고, 종료 후 합본은 도난·중단 세션(가장 중요한 케이스)에서 만들어지지 않음. 공유용 내보내기만 요청 시점에 AVAssetExportSession 1회.
...
### M8 — 온보딩·현지화·수익화 배선
**산출물:** Mac 온보딩 3단계(카메라 권한→iCloud 확인→iPhone 앱 QR, 마지막에 "자리 뜰 땐 화면 잠금" 권장 문구 — §8.6), 한/영 String Catalog 전면 적용, 팁 자(StoreKit 2, Mac 팝오버·iOS 보관함 하단), `HouseBanner`(v1.0은 "Pro 곧 출시" 없이 숨김 처리 가능한 슬롯만). ~~감시 종료 인증 옵션(S3)~~ → **v1.1로 연기 결정 (2026-07-05):** PRD §5가 S3를 Should(MVP 직후)로 분류, 잠금 화면에선 전역 단축키가 동작하지 않아 위협 이미 완화, 사이렌 "3초 내 해제"(§8.10)와 인증의 긴장 관계는 v1.1에서 함께 설계.
...
3. **M8**: ~~종료 인증(S3)을 MVP에 넣을지~~ → **결정됨 (2026-07-05): v1.1로 연기** (근거는 M8 산출물 항목 참고)
